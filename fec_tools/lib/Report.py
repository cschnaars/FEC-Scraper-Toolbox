...
class Report:
    ...
    def __init__(self, rpt_id, down_now=settings.DOWN_NOW, mem_load=settings.MEM_LOAD,
        ...
        upon instantiation. Set to None if the file should not be loaded upon instantiation. If this value matches a key
        found in settings.FILE_TYPES, the class can determine db_load_url automatically.
        :type db_load_delim: str

        :param db_load_url: The URL (or URL pattern) used to download -- and load into the database -- a delimited text
        file in the format specified by db_load_delim. If db_load_delim matches a key found in settings.FILE_TYPES, you
        can set db_load_url to None and the class will determine the URL automatically. This setting is ignored if
        db_load_delim is set to None.
    ...
    def load_to_mem(filetype=settings.DEFAULT_DELIM, url=None, file_path=None, local=True, reload=False):
        """
