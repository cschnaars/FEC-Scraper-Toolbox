...
from FEC_Toolbox.settings.local import DOWNLOAD_TRIES, DOWNLOAD_OVERWRITE, DOWNLOAD_VERIFY, DOWNLOAD_CHUNK_SIZE,\
    REPORTS_DIR, DEFAULT_FILE_TYPE, FILE_TYPES
from FEC_Toolbox.build.lib.get_delimited_report_url import get_delimited_report_url
...
class Report:
    ...
    Report base class.
    """
    def __init__(self, report_id): # , download_url=None, save_path=None):
        self.report_id = report_id
        # self.download_url = download_url
        # self.save_path = save_path

    def download(self, url=None, save_path=None, download_tries=DOWNLOAD_TRIES, overwrite_files=DOWNLOAD_OVERWRITE,
                 chunk_size=DOWNLOAD_CHUNK_SIZE, check_file_length=DOWNLOAD_VERIFY, file_type=DEFAULT_FILE_TYPE):
        """
        ...
        :param overwrite_files: Overwrite existing file. When set to True, the function will overwrite existing files
        ...
        around this problem, report downloads are chunked. Set this value to the number of bytes that should be
        ...
        the function will delete the downloaded file and might try to download it again, depending on the tries setting.
        :type check_file_length: bool

        :param file_type: File type to be downloaded. This setting can be used to determine the download url, the data
        delimiter, the save path and the save file extension, provided file_type has a corresponding key in FILE_TYPES.
        :type file_type: str
        ...
        # Generate save path if does not exist and not provided
        if save_path is None:
            if file_type is None:
                return False, 'Report not downloaded. You must provide a full save path for the file or a file type ' \
                              'contained in FILE_TYPES.'
            if file_type not in FILE_TYPES.keys():
                return False, 'Report not downloaded. You must provide a full save path for the file or a file type ' \
                              'contained in FILE_TYPES.'
            save_path = FILE_TYPES[file_type]['save_path'] + self.report_id + FILE_TYPES[file_type]['file_extension']

        # Generate the url, if not provided
        if url is None:
            url = get_delimited_report_url(self.report_id)
            print('URL: ' + url)
        ...
        if not overwrite_files:
            if os.path.isfile(save_path):
                return False, 'Report not downloaded. File already exists, and overwriting the file is not allowed. ' \
                              'To override this behavior, delete the file or set overwrite_files to True.'
        ...
        for i in range(int(download_tries)):
            try:
                ...
                save_length = os.path.getsize(save_path)
                ...
                try:
                    ...
                    return False, 'File not downloaded. Directory does not exist, and insufficient permissions to ' \
                                  'create.'

            except PermissionError:
                return False, 'File not downloaded. The application might lack permission to write to this ' \
                              'directory, or the file might already exist as read only.'

        # If we get to this point, the file was not downloaded
        return False, 'The report could not be downloaded. Check the url (' + url + ') and save path (' + save_path + \
                      ').'
...
#     Report base class.
#     """
#     def __init__(self, rpt_id, down_now=False):
#         # down_now_urls=None
#         # down_now_paths=None
...
#
#         :param rpt_id: Report ID. Required. This is the ID assigned by the FEC for a specific report. It is used as
#         the primary key for reports in the django database.
#         :type rpt_id: int
#
...
#         :type down_now: dict
#
#         # :param down_now_urls: Download now URLs. You can ignore this setting (set to None) unless a value in
#         # down_now does not match a key in settings.DOWN_NOW or you want to override those settings. In those cases,
#         # set down_now_urls to a list containing a valid URL for each filetype in down_now. For any filetype that is a
#         # key in settings.FILE_TYPES, you can set the appropriate URL in down_now_urls to None, if you want to use the
#         # default URL.
#         # :type down_now_urls: list
#         #
#         # :param down_now_paths: Download now file paths. You can ignore this setting (set to None) unless a filetype
#         # in down_now does not match a key in settings.DOWN_NOW or you want to override those settings. In those
#         # cases, set down_now_paths to a list containing a valid save file path for each filetype in down_now. For any
#         # filetype that is a key in settings.FILE_TYPES, you can set the appropriate path in down_now_paths to None,
#         # if you want to use the default path. Note that unless overridden, the default save path for a given filetype
#         # is: settings.FILE_TYPES[filetype]['save_path']<rpt_id>.settings.FILE_TYPES[filetype]['ext']
...
#         # settings.FILE_TYPES, you can set db_load_url to None and the class will determine the URL automatically.
#         # This setting is ignored if db_load_delim is set to None.
#         # :type db_load_url: str
#         #
#         # :param destroy: Garbage collect this object after instantiation. This is useful if you only want to download
#         # a report and/or load its data into the database but do not otherwise need to inspect the data. Default is
#         # False.
#         # :type destroy: bool
#         #
#         # :return: Report
...
#         """
#         If save path and/or url set to None, retrieve values from FILE_TYPES dictionary. Alert user and exit if
#         filetype does not exist in FILE_TYPES to lookup values that have not been provided. Before downloading, check
#         to see if file already exists. Alert user and take no other action if overwrite is set to False. Otherwise,
#         download the file.
#         :param rpt_id:
#         :param filetype:
#         :param url_pattern:
...
#         Load the raw data. If data is not None and reload is False, alert the user. If filetype is provided, it must
#         be in FILE_TYPES, or user must provide url or file_path. If both url and file_path are available (either from
#         the method call or by looking up those values in FILE_TYPES), the method will first try to get the data from
#         an already downloaded file if local is True. Otherwise, it will try to download the data from the FEC first.
#         User will get an alert if both methods fail.
#         :param filetype:
#         :param url:
#         :param file_path:
#         :param local:
...
#         """
#         Returns URL that be used to view the report online. Note that while this method only returns the URL, there
#         also is a view_url method.
#
#         :return:
#         """
#         pass
...
#         rows to determine the form type and header version, then passes these values to create the appropriate
#         subclass, each of which must override the parent Report class' parse method.
#
#         :return:
#         """
#         pass
